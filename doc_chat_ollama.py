...
def ingest(
    ...
    model: str = typer.Option(DEFAULT_MODEL_NAME, "--model", help="Embedding model"),
    provider: str = typer.Option(DEFAULT_PROVIDER, "--provider", help="Embedding provider: openai or ollama"),
    ...
    ingest_files(dir, db, model, provider, recursive)
...
def search_embeddings(query: str,
                      ...
                      model_name: str,
                      top_k: int,
                      provider: str = DEFAULT_PROVIDER) -> list:
    """
    Search the SQLite DB for the top_k documents most similar to the query.
    """
    # Compute query embedding with OpenAI or Ollama
...
def chat(
    ...
    model: str = typer.Option(DEFAULT_MODEL_NAME, "--model", help="Embedding model"),
    provider: str = typer.Option(DEFAULT_PROVIDER, "--provider", help="Embedding provider: openai or ollama"),
    chat_model: str = typer.Option(DEFAULT_CHAT_MODEL, "--chat-model", help="Chat model for OpenAI or Ollama"),
    ...
    while True:
        ...
        try:
            ...
            if msg.get("function_call"):
                ...
                # Send the function result back to Ollama
            ...
            typer.echo(f"[Error] Chat API call failed: {e}")
