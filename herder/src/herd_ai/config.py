...
def get_model_for_file(file_path, provider="ollama"):
    ...
    if is_code_extension(file_path):
        if provider == "ollama":
            ...
            return XAI_TEXT_MODEL  # Use the default X.AI text model for code
        elif provider == "gemini":
            return GEMINI_TEXT_MODEL  # Use the default Gemini text model for code
        elif provider == "openai":
